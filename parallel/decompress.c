#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <sys/stat.h>
#include <sys/wait.h>
#include <unistd.h>
#include <time.h>
#include <errno.h>

#ifdef _WIN32
#include <direct.h>
#define MKDIR(path) _mkdir(path)
#else
#define MKDIR(path) mkdir(path, 0755)
#endif

#include "huffman.h"

/* ─── Bloque de datos de un archivo comprimido ──────────────────────────── */
typedef struct {
    char     rel_path[PATH_BUF_SIZE];
    uint64_t original_size;
    uint64_t freq[256];
    uint8_t *data;       /* bytes comprimidos */
    size_t   data_len;   /* cantidad de bytes comprimidos */
} FileBlock;

/*
 * Calcula cuantos bytes ocupa la parte comprimida de un archivo dado
 * su tabla de frecuencias (reconstruye el arbol y suma long. de codigos).
 */
static size_t compute_compressed_bytes(const uint64_t freq[256]) {
    HuffmanNode *root = build_huffman_tree(freq);
    if (!root) return 0;
    HuffCode codes[256];
    memset(codes, 0, sizeof(codes));
    build_codes(root, codes, 0, 0);
    free_huffman_tree(root);

    uint64_t total_bits = 0;
    for (int i = 0; i < 256; i++)
        total_bits += freq[i] * (uint64_t)codes[i].length;
    return (size_t)((total_bits + 7) / 8);
}

/* ─── Proceso hijo: decodifica un FileBlock y escribe su archivo ─────────── */
static void child_decode(const char *output_dir, const FileBlock *block) {
    char out_path[PATH_BUF_SIZE];
    snprintf(out_path, sizeof(out_path), "%s/%s", output_dir, block->rel_path);

    if (!ensure_parent_dirs(out_path)) {
        fprintf(stderr, "No se pudieron crear directorios para %s\n", out_path);
        exit(1);
    }

    FILE *out = fopen(out_path, "wb");
    if (!out) { perror(out_path); exit(1); }

    if (block->original_size == 0) {
        fclose(out);
        printf("Recuperado: %s\n", block->rel_path);
        exit(0);
    }

    HuffmanNode *root = build_huffman_tree(block->freq);
    if (!root) { fclose(out); exit(1); }

    /* fmemopen: crea un FILE* de lectura sobre el buffer en memoria */
    FILE *mem_in = fmemopen(block->data, block->data_len > 0 ? block->data_len : 1, "rb");
    if (!mem_in) { perror("fmemopen"); free_huffman_tree(root); fclose(out); exit(1); }

    BitReader br;
    bit_reader_init(&br, mem_in);

    /* Caso especial: arbol con un solo simbolo */
    if (root->left && !root->right && root->left->is_leaf) {
        for (uint64_t i = 0; i < block->original_size; i++)
            fputc(root->left->byte, out);
        uint64_t pad_bits = (8 - (block->original_size % 8)) % 8;
        for (uint64_t i = 0; i < pad_bits; i++)
            if (bit_reader_read_bit(&br) < 0) break;
        free_huffman_tree(root);
        fclose(mem_in); fclose(out);
        printf("Recuperado: %s\n", block->rel_path);
        exit(0);
    }

    /* Decodificacion normal */
    for (uint64_t written = 0; written < block->original_size; written++) {
        HuffmanNode *curr = root;
        while (curr && !curr->is_leaf) {
            int bit = bit_reader_read_bit(&br);
            if (bit < 0) { free_huffman_tree(root); fclose(mem_in); fclose(out); exit(1); }
            curr = (bit == 0) ? curr->left : curr->right;
        }
        if (!curr) { free_huffman_tree(root); fclose(mem_in); fclose(out); exit(1); }
        fputc(curr->byte, out);
    }

    free_huffman_tree(root);
    fclose(mem_in);
    fclose(out);
    //printf("Recuperado: %s\n", block->rel_path);
    exit(0);
}

/* ─── main ──────────────────────────────────────────────────────────────── */
int main(int argc, char *argv[]) {
    if (argc != 3) {
        fprintf(stderr, "Uso: %s <archivo_entrada.bin> <directorio_salida>\n", argv[0]);
        return 1;
    }
    const char *input_file = argv[1];
    const char *output_dir  = argv[2];

    struct timespec ts_start, ts_end;
    clock_gettime(CLOCK_MONOTONIC, &ts_start);

    MKDIR(output_dir); /* si ya existe, no importa */

    FILE *in = fopen(input_file, "rb");
    if (!in) { perror(input_file); return 1; }

    char magic[5] = {0};
    if (fread(magic, 1, 4, in) != 4 || strncmp(magic, HUFF_MAGIC, 4) != 0) {
        fprintf(stderr, "Archivo invalido o formato no soportado.\n");
        fclose(in); return 1;
    }

    uint32_t file_count;
    if (!read_u32(in, &file_count)) { fclose(in); return 1; }

    /* 1. Leer TODOS los bloques en memoria (lectura secuencial obligatoria del .bin) */
    FileBlock *blocks = calloc(file_count, sizeof(FileBlock));
    if (!blocks) { perror("calloc"); fclose(in); return 1; }

    for (uint32_t i = 0; i < file_count; i++) {
        uint16_t path_len;
        if (!read_u16(in, &path_len) || path_len >= PATH_BUF_SIZE) {
            fprintf(stderr, "Error leyendo bloque %u\n", i);
            fclose(in); free(blocks); return 1;
        }
        if (fread(blocks[i].rel_path, 1, path_len, in) != path_len) {
            fclose(in); free(blocks); return 1;
        }
        blocks[i].rel_path[path_len] = '\0';

        if (!read_u64(in, &blocks[i].original_size)) { fclose(in); free(blocks); return 1; }

        for (int j = 0; j < 256; j++)
            if (!read_u64(in, &blocks[i].freq[j])) { fclose(in); free(blocks); return 1; }

        blocks[i].data_len = compute_compressed_bytes(blocks[i].freq);
        if (blocks[i].data_len > 0) {
            blocks[i].data = malloc(blocks[i].data_len);
            if (!blocks[i].data) { perror("malloc"); fclose(in); free(blocks); return 1; }
            if (fread(blocks[i].data, 1, blocks[i].data_len, in) != blocks[i].data_len) {
                fprintf(stderr, "Error leyendo datos comprimidos del bloque %u\n", i);
                fclose(in); free(blocks); return 1;
            }
        } else {
            blocks[i].data = NULL;
        }
    }
    fclose(in);

    /* 2. Lanzar un hijo por archivo */
    pid_t *pids = malloc(file_count * sizeof(pid_t));
    if (!pids) { perror("malloc"); free(blocks); return 1; }

    for (uint32_t i = 0; i < file_count; i++) {
        pids[i] = fork();
        if (pids[i] < 0) {
            perror("fork");
            for (uint32_t j = 0; j < i; j++) waitpid(pids[j], NULL, 0);
            free(pids); free(blocks); return 1;
        }
        if (pids[i] == 0) {
            /* ── HIJO ── */
            child_decode(output_dir, &blocks[i]);
            /* no retorna */
        }
    }

    /* 3. Padre espera a todos los hijos */
    int all_ok = 1;
    for (uint32_t i = 0; i < file_count; i++) {
        int status;
        waitpid(pids[i], &status, 0);
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
            fprintf(stderr, "Error en proceso hijo para: %s\n", blocks[i].rel_path);
            all_ok = 0;
        }
    }
    free(pids);
    for (uint32_t i = 0; i < file_count; i++) free(blocks[i].data);
    free(blocks);

    clock_gettime(CLOCK_MONOTONIC, &ts_end);
    long elapsed_ms = (ts_end.tv_sec - ts_start.tv_sec) * 1000L
                    + (ts_end.tv_nsec - ts_start.tv_nsec) / 1000000L;
    printf("Descompresion completada en: %s\n", output_dir);
    printf("Tiempo de ejecucion: %ld ms\n", elapsed_ms);
    return all_ok ? 0 : 1;
}
